...
import { Check, X, Ticket, Crown, Search, Filter, Download } from 'lucide-react';
...
const GuestPage = ({ id, event }) => {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      alert(
        `❌ Check-in failed: ${err?.response?.data?.message || err.message || 'Unknown error'}`,
      );
    } finally {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      await axios.post(
        `${urlBe}/events/guests/${selectedGuestId}/decline`,
        {},
        {
          headers: { 'x-jdticket': localStorage.getItem('token') || '' },
        },
      ...
      try {
        await axios.put(
          `${urlBe}/events/guests/${selectedGuestId}/status`,
          { status: 'DECLINED' },
          {
            headers: { 'x-jdticket': localStorage.getItem('token') || '' },
          },
        ...
        alert(
          `❌ Decline failed: ${err?.response?.data?.message || err.message || e?.message || 'Unknown error'}`,
        );
      }
    } finally {
      setActionLoading(false);
  ...
  const handleDeclineFromList = async (guest) => {
    ...
    try {
      await axios.post(
        `${urlBe}/events/guests/${guest.id}/decline`,
        {},
        {
          headers: { 'x-jdticket': localStorage.getItem('token') || '' },
        },
      ...
      try {
        await axios.put(
          `${urlBe}/events/guests/${guest.id}/status`,
          { status: 'DECLINED' },
          {
            headers: { 'x-jdticket': localStorage.getItem('token') || '' },
          },
            ...
            >
              <div className="flex items-center gap-3">
                <img
                  src={
                    guest.image ||
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(guest.name)}&background=random`
                  }
                  alt={guest.name}
                  className="w-8 h-8 rounded-full object-cover"
                />
                <div className="font-['Satoshi-Medium',_sans-serif]">{guest.name}</div>
                <div className="text-sm text-gray-400 font-['Satoshi-Regular',_sans-serif]">
                  {guest.email}
                </div>
              </div>
            ...
            {detailLoading ? (
              ...
              <div>
                ...
                <div className="flex gap-4 items-center mb-4">
                  <img
                    src={
                      selectedGuestDetail.image ||
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedGuestDetail.name || selectedGuestDetail.email)}&background=random`
                    }
                    alt={selectedGuestDetail.name}
                    className="w-16 h-16 rounded-full object-cover"
                  />
                  <div>
                    <div className="text-xl font-['Satoshi-Medium',_sans-serif]">
                      {selectedGuestDetail.name || selectedGuestDetail.email}
                    </div>
                ...
                <div className="grid grid-cols-3 gap-6 mb-4">
                  <div>
                    ...
                    <div className="font-['Satoshi-Medium',_sans-serif]">
                      {formatDate(
                        selectedGuestDetail.create_date || selectedGuestDetail.created_at,
                      )}
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-['Satoshi-Medium',_sans-serif]">
                      {selectedGuestDetail.total_ticket
                        ? `${selectedGuestDetail.total_ticket} Tickets`
                        : '-'}
                    </div>
                  </div>
                </div>
                ...
                {selectedGuestDetail.is_checkin ? (
                  <div className="bg-green-800 text-green-100 rounded-md p-3 mb-4">
                    User already checked in, check in date:{' '}
                    {formatDate(
                      selectedGuestDetail.checkin_date || selectedGuestDetail.checkin_date,
                    )}
                  </div>
                ) : (
                  <>
                    <div className="bg-yellow-800 text-yellow-100 rounded-md p-3 mb-4">
                      This guest is currently pending approval
                    </div>
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="text-lg font-['Satoshi-Medium',_sans-serif]">
                      {selectedGuestDetail.total_ticket} Tickets
                    </div>
                    <div className="text-sm text-gray-400">
                      {selectedGuestDetail.invoice_code
                        ? `Invoice: ${selectedGuestDetail.invoice_code}`
                        : ''}
                    </div>
                  </div>
                ...
                {/* Additional info */}
                <div>
                  <div className="text-lg font-['Satoshi-Medium',_sans-serif] mb-2">
                    Additional Question
                  </div>
                  <div className="bg-[#111] rounded-md p-4">
                    <div className="text-sm">
                      {selectedGuestDetail.additional_questions
                        ? JSON.stringify(selectedGuestDetail.additional_questions, null, 2)
                        : '-'}
                    </div>
                  </div>
                </div>
