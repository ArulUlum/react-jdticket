  ...
  return (
    <div className="min-h-screen pb-5">
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <div className="text-white text-4xl md:text-5xl font-bold mb-4">Rp {balance.toLocaleString("id-ID")}</div>
        <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-[#181818] rounded-2xl shadow-xl p-8 w-full max-w-md relative border border-[#232323]">
            ...
            <p className="text-[#a2a2a2] mb-6">Enter the amount you wish to withdraw and confirm with your password.</p>
            <div className="mb-4">
              <label className="block text-white mb-1">Amount</label>
              <div className="flex items-center bg-[#232323] rounded-lg px-3 py-2">
                <span className="text-white text-lg font-bold flex-1">Rp {withdrawAmount.toLocaleString("id-ID")}</span>
                <button
            ...
            <button
              ...
              onClick={() => {/* handle withdrawal logic here */}}
            >
              Proceed Withdrawal
            </button>
          </div>
      ...
      <div className="flex flex-wrap gap-4 mb-10">
        {eventBalances.map(ev => (
          <div key={ev.name} className="bg-[#141717] rounded-xl p-5 min-w-[220px] flex-1">
            ...
            <div className="text-white text-xl font-bold mb-1">Rp {ev.balance.toLocaleString("id-ID")}</div>
            <div className="text-[#a2a2a2] text-sm">Tax Balance : Rp {ev.tax.toLocaleString("id-ID")}</div>
          </div>
        ))}
      </div>
      ...
      {payments && payments.length > 0 ? (
              ...
              {payments.map((p, idx) => (
                <tr key={idx} className="rounded-lg text-white text-sm">
                  ...
                  <td className="px-3 py-2">
                    <span className="bg-green-700 text-white rounded px-3 py-1 text-xs font-semibold">{p.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex flex-col items-center justify-center mt-10">
          <svg width="120" height="120" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="60" y="65" textAnchor="middle" fill="#a2a2a2" fontSize="24" fontWeight="bold">$</text>
            <text x="90" y="97" textAnchor="middle" fill="#a2a2a2" fontSize="18">🕒</text>
          </svg>
